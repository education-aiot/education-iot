...
class MainStudent(QWidget, ui):
    ...
    def __init__(self):
        ...
        self.consulting_btn.clicked.connect(self.invitemess)
    ...
    def receive_messages(self, sock): # 메시지 받기
        ...
        while True:
            ...
            elif '채팅초대' in self.final_message:

                # QmessageBox로 yes or no 판별
                invite=self.QMessageBox.question("초대요청", "상당방에 초대받으셨습니다. ",QMessageBox.Yes|QMessageBox.No,QMessageBox.Yes)

                if invite==QMessageBox.Yes:
                    self.move_page('상담방')
                elif invite==QMessageBox.No:
                    self.sock.send('수락거절'.encode())
    ...
    def sendqna(self): # 질문 보내기
        ...
        sendData = f'Qusetion/{self.send_line.text()}'
